...
import static com.cgq.cardslide.view.DrawHelperUtils.drawPartCircle;
...
public class SwipeCaptchaView extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        ta.recycle();
        mRandom = new Random(Math.abs(System.nanoTime()));
        //初始化画笔
...
/*        mMaskShadowPaint.setStrokeWidth(50);
    ...
    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {//View大小发生变化会被回调
    ...
    private void createMatchAnim() {
        ...
        mFailAnim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                //在动画结束后执行监听回调通知验证失败的结果
                onCaptchaMatchCallback.matchFailed(SwipeCaptchaView.this);
            }
        });
        mFailAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {//监听动画的每一帧更新的时候调用
            @Override
        ...
        mSuccessPaint.setShader(new LinearGradient(0, 0, width / 2 * 3, mHeight, new int[]{0x00ffffff, 0x88ffffff}, new float[]{0, 0.5f},
